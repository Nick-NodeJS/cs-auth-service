...
pub async fn auth_callback(
    ...
    if let Some(mut user_session) = user_service
        .get_user_session(tokens.clone(), UserProfile::Google(user_profile.clone()))
        .await?
    {
        log::info!(
        ...
        // TODO: check if it has correct USER_AGENT on this step
        user_session.metadata.set_metadata_from_request(&req);
